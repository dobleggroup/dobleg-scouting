...
/** Scatter de dos métricas: pool + protagonista resaltado, con cuadrante "mejores". */
export default function InformeScatter({ scatter, matrix, defs, protagonistIndex, lang }: InformeScatterProps) {
    ...
    <div>
      <div className="flex items-center gap-5 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-apple-gray-300">{t(lang, 'l_thisPlayer')}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3.5 h-3.5 rounded-full flex-shrink-0" style={{ backgroundColor: 'rgba(148,163,184,0.5)' }} />
          <span className="text-sm text-apple-gray-300">{t(lang, 'l_otherPlayers')}</span>
        </div>
      </div>
