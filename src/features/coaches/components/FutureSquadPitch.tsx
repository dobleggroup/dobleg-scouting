        ...
        return (
          ...
          >
            ...
            <div className="relative w-12 h-12 sm:w-16 sm:h-16">
              ...
              {/* "+": este puesto admite más opciones (hasta 6). Abre el mismo selector. */}
              {occupied && 1 + (slot!.alternates?.length ?? 0) < 6 && (
                <button
                  type="button"
                  onClick={() => onSlotClick(pos.key)}
                  aria-label={t('futureSquadPitch.agregarOpcion')}
                  title={t('futureSquadPitch.agregarOpcion')}
                  className="absolute -top-1 -left-1 w-5 h-5 rounded-full bg-white text-emerald-700 flex items-center justify-center shadow-md ring-1 ring-emerald-700/20 hover:scale-110 transition-transform"
                >
