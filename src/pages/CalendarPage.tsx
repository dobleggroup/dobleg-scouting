...
function MatchPill({ fixture }: { fixture: AgencyFixture }) {
  ...
  const ourTeam = fixture.isHome ? fixture.homeTeam : fixture.awayTeam
  const opponent = fixture.isHome ? fixture.awayTeam : fixture.homeTeam

  return (
    <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-2xs leading-tight truncate ${
      live ? 'bg-brand-green/15 text-brand-green font-semibold' :
      done ? 'bg-apple-gray-100 dark:bg-apple-gray-700/50 text-apple-gray-500 dark:text-apple-gray-400' :
      'bg-brand-green/8 text-apple-gray-700 dark:text-apple-gray-300'
    }`}>
      <img src={opponent.logo} alt="" className="w-3 h-3 object-contain flex-shrink-0" />
      <span className="truncate">
        {done ? `${fixture.goalsHome}-${fixture.goalsAway}` : formatTime(fixture.date)}
        {' '}{fixture.isHome ? 'vs' : '@'} {opponent.name.length > 12 ? opponent.name.slice(0, 11) + '…' : opponent.name}
      </span>
      {live && <span className="w-1 h-1 rounded-full bg-brand-green animate-pulse-soft flex-shrink-0" />}
    </div>
  )
}
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-apple transition-colors hover:bg-apple-gray-50 dark:hover:bg-apple-gray-700/30 ${
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        <img src={fixture.homeTeam.logo} alt="" className="w-6 h-6 object-contain flex-shrink-0" />
        ...
        <img src={fixture.awayTeam.logo} alt="" className="w-6 h-6 object-contain flex-shrink-0" />
      </div>
      ...
      <span className={`text-2xs flex-shrink-0 ${fixture.isHome ? 'text-blue-500' : 'text-orange-400'}`}>
        {fixture.isHome ? 'L' : 'V'}
      </span>
    </div>
  )
          ...
          {week.map(day => {
            ...
            return (
              <button
                ...
                className={`min-h-[80px] sm:min-h-[100px] p-1 sm:p-1.5 text-left border-r last:border-r-0 border-apple-gray-100 dark:border-apple-gray-700/20 transition-colors ${
              ...
              >
                ...
                <div className="space-y-0.5 hidden sm:block">
                  ...
                  {matches.length > 3 && (
                    <span className="text-2xs text-apple-gray-400 pl-1">+{matches.length - 3} más</span>
                  )}
                </div>
                {/* Mobile: just dots */}
